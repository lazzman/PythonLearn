...
def main():
    '''
    socket.socket()默认使用的是tcp协议，ipv4地址簇
    方法签名：
    ...
    参数二：类型
    socket.SOCK_STREAM 流式socket，for TCP协议(默认)
    socket.SOCK_DGRAM 数据报式socket，for UDP协议
    ...
    socket.SOCK_DRW 是一种可靠的UDP形式，即保证交付数据报但不保证顺序。SOCK_DRW用来提供对原始协议的低级访问，在需要执行某些特殊操作时使用，如发送ICMP报文。
